...
class RepublicSpider(scrapy.Spider):
    ...
    def parse(self, response):
        list = response.xpath(
            '//a[@href ] /div[@class="card-headline txtTruncate"]/..')
        list2 = response.xpath(
            '//a[@href]/span[@class="card-headline txtTruncate "]/..')
        # listheadline = []
        # listlink = []
        #
        ...
        try:
            for li in list:
                headline1 = (
                    li.xpath('.//div[@class="card-headline txtTruncate"]/text()').extract_first())
            ...
            for li in list2:
                headline1 = (li.xpath(
                    './/span[@class="card-headline txtTruncate "]/text()').extract_first())
                href = (li.xpath('.//@href').extract_first())
                if type(headline1) is str:
                    headline1 = headline1.strip()
                    print(headline1)
